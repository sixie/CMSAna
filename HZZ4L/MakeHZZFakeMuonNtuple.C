...
//=== MAIN MACRO =================================================================================================
void MakeHZZFakeMuonNtuple(Int_t Sample = 100) {
...
void MakeHZZFakeMuonNtuple(const string inputFilename, const string outputFilename) {
  ...
  for(UInt_t ientry=0; ientry < eventTree->GetEntries(); ientry++) {       	
    ...
    for(Int_t i=0; i<genjetArr->GetEntries(); i++) {
      ...
      for(Int_t j=0; j<muUsedEta.size(); j++) {
        if (cmsana::deltaR(genjet->eta,genjet->phi,muUsedEta[j],muUsedPhi[j]) < 0.1) {
          isDuplicate = true;
          break;
        }
      }
      ...
      for(Int_t j=0; j<muonArr->GetEntries(); j++) {
        ...
        if (cmsana::deltaR(genjet->eta,genjet->phi,mu->eta,mu->phi) < 0.1) {
          mupt = 0;
      ...
      muTree->fRho = rho;
      muTree->fMuGenPt = genjet->pt; 
      muTree->fMuGenEta = genjet->eta; 
      muTree->fMuGenPhi = genjet->phi; 
      muTree->fMuPt = mupt;
      muTree->fMuEta = mueta;
      muTree->fMuPhi = muphi;
      muTree->fPdgId = genjet->matchedPdgId;
      muTree->tree_->Fill();
      NMuonsFilled++;
