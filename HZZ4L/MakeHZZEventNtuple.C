...
void DoMakeHZZEventNtuple(const string inputfilename, const string Label = "ZZ") 
{  
  string label = Label;
  ...
  cout << "Total Events: " << eventTree->GetEntries() << "\n";
...
//     cout << "ZZ: " << (genLep1v+genLep2v+genLep3v+genLep4v).M() << " " << (genLep1v+genLep2v+genLep3v+genLep4v).Pt() << " " << (genLep1v+genLep2v+genLep3v+genLep4v).Eta() << "\n";
    ...
    //GenLep1
    if (abs(genLep1->pdgid) == 11) {
      for(uint i=0; i<uint(electronArr->GetEntries()); i++) 
      {
        const cmsana::TElectron *ele = (cmsana::TElectron*)((*electronArr)[i]);
        if (cmsana::deltaR(genLep1->eta,genLep1->phi,ele->eta,ele->phi) < 0.1) {
          Lep1v.SetPt(ele->pt);
          Lep1v.SetEta(ele->eta);
          Lep1v.SetPhi(ele->phi);
      ...
      for(uint i=0; i<uint(muonArr->GetEntries()); i++) 
      {
        ...
        if (cmsana::deltaR(genLep1->eta,genLep1->phi,mu->eta,mu->phi) < 0.1) {
          Lep1v.SetPt(mu->pt);
          Lep1v.SetEta(mu->eta);
          Lep1v.SetPhi(mu->phi);
          Lep1v.SetM(genLep1->mass);
    ...
    //GenLep2
    if (abs(genLep2->pdgid) == 11) {
      for(uint i=0; i<uint(electronArr->GetEntries()); i++) 
      {
        const cmsana::TElectron *ele = (cmsana::TElectron*)((*electronArr)[i]);
        if (cmsana::deltaR(genLep2->eta,genLep2->phi,ele->eta,ele->phi) < 0.1) {
          Lep2v.SetPt(ele->pt);
          Lep2v.SetEta(ele->eta);
          Lep2v.SetPhi(ele->phi);
      ...
      {
        const cmsana::TMuon *mu = (cmsana::TMuon*)((*muonArr)[i]);        
        if (cmsana::deltaR(genLep2->eta,genLep2->phi,mu->eta,mu->phi) < 0.1) {
          Lep2v.SetPt(mu->pt);
          Lep2v.SetEta(mu->eta);
          Lep2v.SetPhi(mu->phi);
          Lep2v.SetM(genLep2->mass);
    ...
    //GenLep3
    if (abs(genLep3->pdgid) == 11) {
      for(uint i=0; i<uint(electronArr->GetEntries()); i++) 
      {
        const cmsana::TElectron *ele = (cmsana::TElectron*)((*electronArr)[i]);
        if (cmsana::deltaR(genLep3->eta,genLep3->phi,ele->eta,ele->phi) < 0.1) {
          Lep3v.SetPt(ele->pt);
          Lep3v.SetEta(ele->eta);
          Lep3v.SetPhi(ele->phi);
      ...
      {
        const cmsana::TMuon *mu = (cmsana::TMuon*)((*muonArr)[i]);        
        if (cmsana::deltaR(genLep3->eta,genLep3->phi,mu->eta,mu->phi) < 0.1) {
          Lep3v.SetPt(mu->pt);
          Lep3v.SetEta(mu->eta);
          Lep3v.SetPhi(mu->phi);
          Lep3v.SetM(genLep3->mass);
    ...
    //GenLep4
    if (abs(genLep4->pdgid) == 11) {
      for(uint i=0; i<uint(electronArr->GetEntries()); i++) 
      {
        const cmsana::TElectron *ele = (cmsana::TElectron*)((*electronArr)[i]);
        if (cmsana::deltaR(genLep4->eta,genLep4->phi,ele->eta,ele->phi) < 0.1) {
          Lep4v.SetPt(ele->pt);
          Lep4v.SetEta(ele->eta);
          Lep4v.SetPhi(ele->phi);
      ...
      {
        const cmsana::TMuon *mu = (cmsana::TMuon*)((*muonArr)[i]);        
        if (cmsana::deltaR(genLep4->eta,genLep4->phi,mu->eta,mu->phi) < 0.1) {
          Lep4v.SetPt(mu->pt);
          Lep4v.SetEta(mu->eta);
          Lep4v.SetPhi(mu->phi);
          Lep4v.SetM(genLep4->mass);
...
void MakeHZZEventNtuple(const string Label = "1125", Int_t Option = 1125) 
{  

  if (Option == 1125) {
    DoMakeHZZEventNtuple("/afs/cern.ch/user/s/sixie/work/public/HZZ4l/ntuples/BACON/V3/BACONNtuple_1125.root", Label);
  }
  if (Option == 102) {
    DoMakeHZZEventNtuple("/afs/cern.ch/user/s/sixie/work/public/HZZ4l/ntuples/BACON/V3/BACONNtuple_102.root", Label);
  }
  if (Option == 103) {
    DoMakeHZZEventNtuple("/afs/cern.ch/user/s/sixie/work/public/HZZ4l/ntuples/BACON/V3/BACONNtuple_103.root", Label);
  }
  if (Option == 105) {
    DoMakeHZZEventNtuple("/afs/cern.ch/user/s/sixie/work/public/HZZ4l/ntuples/BACON/V3/BACONNtuple_105.root", Label);
  }
