...
//
//root -l -b -q  CMSAna/HHToBBGG/HHToBBGGSelectionFakePhotons.C+'("root://eoscms//eos/cms/store/user/sixie/BACON/V00-00-02/HHtoBBGG-14tev-START53_V7A/BACONNtuple.dihiggs-bbgg-14tev.1.root","HHToBBGG.root",1)'
//root -l -b -q  CMSAna/HHToBBGG/HHToBBGGSelectionCCFakePhotons.C+'("root://eoscms//eos/cms/store/user/sixie/BACON/V00-00-00/ttHgg-125-START53_V7A/BACONNtuple_1_1_CaY.root","ccgg.root",2)'
//root -l -b -q  CMSAna/HHToBBGG/HHToBBGGSelectionCCFakePhotons.C+'("BACONNtuple.root","test.root",0)'
//
...
void HHToBBGGSelectionCCFakePhotons(const string inputfile,          // input file
  ...
  TH1F *N4genjets = new TH1F("N4genjets", "Number of Events w/ at 2 bjets and >=2 genjets", 1, -0.5, 0.5); // Count the number of events that have two real cjets + at least two other jets
...
//            << " | " << p->motherPdgID << "\n";
    ...
    // Discount all events with any real photons and real bjets
    ...
    for(Int_t i=0; i<genjetArr->GetEntriesFast(); i++) {
      ...
      if (genjet1 == genBJet1 || genjet1 == genBJet2) continue;
      if (genjet1->pt < 20) continue;
      if (fabs(genjet1->eta) > 2.5) continue;
      
      //select second genjet
	...
	photon1 = genjet1;
	photon1v.SetPt(genjet1->pt);
	photon1v.SetEta(genjet1->eta);
	photon1v.SetPhi(genjet1->phi);
	photon1v.SetM(0);
	photonsPt+=genjet1->pt;
	...
	for(Int_t k=0; k<genjetArr->GetEntriesFast(); k++) {
	  ...
	  if (othergenjet == photon1 || othergenjet == photon2 || othergenjet == genBJet1 ||othergenjet == genBJet2) continue;
