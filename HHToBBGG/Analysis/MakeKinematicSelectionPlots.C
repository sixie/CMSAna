...
void MakeKinematicSelectionPlots ( string InputFilename    = "/data1/sixie/Phase2Upgrade/HHtoBBGG/normalizedNtuples/V00-00-04/HHToBBGGNtuple.AllProcessesCombined.root")
{
  ...
  for (int n=0;n<event.tree_->GetEntries();n++) { 
    ...
    if (event.bjet1.Pt() > 30.0 && event.bjet2.Pt() > 30.0
      ...
      if (event.diphoton.M() > 100 && event.diphoton.M() < 150
        ...
        if (event.DRgg < 2.0 && event.minDRgb > 1.5) {
  ...
  //*******************************************************************************************
  cv = new TCanvas("cv","cv", 800,600);
  legend = new TLegend(0.75,0.54,0.90,0.84);
  legend->SetTextSize(0.03);
  legend->SetBorderSize(0);
  legend->SetFillStyle(0);
  ...
  legend->Draw();
  cv->SaveAs("dRgg_ps_normalized.gif");
  ...
  //*******************************************************************************************
  cv = new TCanvas("cv","cv", 800,600);
  legend = new TLegend(0.65,0.54,0.80,0.84);
  legend->SetTextSize(0.03);
  legend->SetBorderSize(0);
  legend->SetFillStyle(0);
  ...
  legend->Draw();
  cv->SaveAs("dRbb_s0_normalized.gif");
  ...
  legend->Draw();
  cv->SaveAs("minDRgb_ps_normalized.gif");
