...
string bkgLegendLabels[7] = { "HH#rightarrow bb#gamma#gamma", 
...
// PlotHiggsRes_LP
//------------------------------------------------------------------------------
void CutBasedAnalysis ( string InputFilename    = "/afs/cern.ch/work/s/sixie/public/HHToBBGG/normalizedNtuples/HHToBBGGNtuple.main.root")
{
  ...
  for (int n=0;n<event.tree_->GetEntries();n++) { 
    ...
    //make sure the sample type makes sense
     assert(event.sampletype >= 1 && event.sampletype <= 7);
...
//      if (event.sampletype != 1) continue;
     ...
     if (event.bjet1.Pt() > 25.0 && event.bjet2.Pt() > 25.0
       ...
       if (event.nlep <= 0 && event.ncentraljets <= 3
  ...
  for (Int_t i = diphotonPt.size()-1; i >= 0; i--) {
    cout << "sample " << i << " : " << diphotonPt[i]->Integral() << "\n";
