...
void FitScenariosTwoD_RequiredPerformance(const string inputfilePho = "/afs/cern.ch/work/d/daan/public/releases/CMSSW_5_3_9_patch3/src/CMSAna/HHToBBGG/data/HHToBBGG_SignalBkgd_AfterCuts_diphotonMass.root", const string inputfileBjet = "/afs/cern.ch/work/d/daan/public/releases/CMSSW_5_3_9_patch3/src/CMSAna/HHToBBGG/data/HHToBBGG_SignalBkgd_AfterCuts_dibjetMass.root", const string inputfileTwoD = "/afs/cern.ch/work/d/daan/public/releases/CMSSW_5_3_9_patch3/src/CMSAna/HHToBBGG/data/HHToBBGG_SignalBkgd_AfterCuts_twoDMass.root", const string scanOption = "lum", Int_t NToys = 500, Int_t s = 5) {
  ...
  TFile *f = new TFile(Form(("CMSAna/HHToBBGG/data/MassFitResults/ResolutionAnalysis/OptionB_Categories/baseline140PU/"+scanOption+"FitScenario%d.root").c_str(), s), "RECREATE");
  ...
  if (scanOption == "phoEff") myPhoEffRatio = phoEffRatio[s];
  if (scanOption == "btagEff") myBtagEffRatio = btagEffRatio[s];
  ...
  for(UInt_t t=0; t < NToys; ++t) {
    ...
    RooDataSet *pseudoData2D_cat0 = model2Dpdf_cat0->generate(RooArgList(*massBjet,*massPho), ran);
    RooDataSet *pseudoData2D_cat1 = model2Dpdf_cat1->generate(RooArgList(*massBjet,*massPho), ran);
...
void AddModels(RooWorkspace *ws, const string inputfilePho, const string inputfileBjet, const string inputfileTwoD, const string scanOption, Int_t s) {
  ...
  datacard << "kmax 2 number of nuisance parameters" << endl;
  datacard << "----------------------------------------------------------------------------------------------------------------------------------" << endl;
