//
// root -l -b -q CMSAna/HHToBBGG/fits/FitMassJERSystematics.C+'("/afs/cern.ch/work/s/sixie/public/releases/analysis/CMSSW_5_3_9_patch3/src/CMSAna/HHToBBGG/data/FitWorkspace.root", "MassFitTwoD_JERDown.root", 1, 1, -1, 5000)' |& tee log_JERDown
//
...
void FitMassJERSystematics(const string workspaceFile = "/afs/cern.ch/work/d/daan/public/releases/CMSSW_5_3_9_patch3/src/CMSAna/HHToBBGG/data/FitWorkspace_asdf.root", const string outputTree = "/afs/cern.ch/work/d/daan/public/releases/CMSSW_5_3_9_patch3/src/CMSAna/HHToBBGG/data/MassFitResults/MassFitTwoD_asdf.root", Int_t plotOption = 0, Int_t storeOption = 1, Int_t SystematicsUpOrDown = 1, Int_t NToys = 5000) {
  ...
  double inputSigSigmaBJet = sigSigmaBjet->getVal();
  double inputResSigmaBJet = resSigmaBjet->getVal();
  ...
  for(UInt_t t=0; t < NToys; ++t) {
    ...
    sigSigmaBjet->setVal(inputSigSigmaBJet);
    resSigmaBjet->setVal(inputResSigmaBJet);

    cout << "Before: " << sigSigmaBjet->getVal() << " , " << resSigmaBjet->getVal() << " | ";
    ...
    if (SystematicsUpOrDown == 1) {
      sigSigmaBjet->setVal(inputSigSigmaBJet*1.1);
      resSigmaBjet->setVal(inputResSigmaBJet*1.1);
    } else if (SystematicsUpOrDown == -1) {
      sigSigmaBjet->setVal(inputSigSigmaBJet/1.1);
      resSigmaBjet->setVal(inputResSigmaBJet/1.1);
    }

    cout << "After: " << sigSigmaBjet->getVal() << " , " << resSigmaBjet->getVal() << " \n";
