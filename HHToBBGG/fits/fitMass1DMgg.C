...
//     RooRealVar nnonresPho("N_{#gamma#gamma} (NonResBkg)", "# non-resonant background events #gamma#gamma", 146.5, 134.5, 158.5);
    RooRealVar nsigPho("N_{#gamma#gamma} (Sig)", "# signal events #gamma#gamma", 38.5, 0, 1000.0);
    RooRealVar nresPho("N_{#gamma#gamma} (ResBkg)", "# resonant background events #gamma#gamma", 0.0, 0, 26.7);
    RooRealVar nnonresPho("N_{#gamma#gamma} (NonResBkg)", "# non-resonant background events #gamma#gamma", 146.5, 0, 2000.0);
    nresPho.setConstant(true);
    //meanPho.setConstant(true);
    //sigSigmaPho.setConstant(true);

    //PDFs for diPhoton
    RooGaussian sigPDFPho("sigPDFPho", "Signal PDF #gamma#gamma", massPho, meanPho, sigSigmaPho);
    RooGaussian resPDFPho("resPDFPho", "Resonant Background PDF #gamma#gamma", massPho, meanPho, sigSigmaPho);
    RooExponential nonresPDFPho("nonresPDFPho", "Non-Resonant Background PDF #gamma#gamma", massPho, expRatePho);
    RooAddPdf modelPho("modelPho", "Signal+Background PDF #gamma#gamma", RooArgList(sigPDFPho, nonresPDFPho), RooArgList(nsigPho, nnonresPho));
    ...
    RooDataSet *pseudoDataPho = modelPho.generate(massPho, randomnumber->Poisson(185.));
...
//     fitResult2D = model2Dpdf.fitTo(*pseudoData2D, RooFit::Save(), RooFit::Extended(kTRUE), RooFit::Strategy(2));
    ...
    */
    ...
    cv = new TCanvas("cv","cv",800,600);
    framey->Draw();
    cv->SaveAs(Form("Plots/AllSignalBkgd/Fits/test_1DFit_Mgg_%d.gif",t));
