...
//
//root -l -b -q  CMSAna/HHToBBGG/HHToBBGGSelectionJJJPhoton.C+'("root://eoscms//eos/cms/store/user/sixie/BACON/V00-00-02/HHtoBBGG-14tev-START53_V7A/BACONNtuple.dihiggs-bbgg-14tev.1.root","HHToBBGG.root",1)'
//root -l -b -q  CMSAna/HHToBBGG/HHToBBGGSelectionJJJPhoton.C+'("root://eoscms//eos/cms/store/user/sixie/BACON/V00-00-00/ttHgg-125-START53_V7A/BACONNtuple_1_1_CaY.root","jjjg.root",2)'
//root -l -b -q  CMSAna/HHToBBGG/HHToBBGGSelectionJJJPhoton.C+'("BACONNtuple.root","test.root",0)'
//
...
void HHToBBGGSelectionJJJPhoton(const string inputfile,          // input file
  ...
  TH1F *Npass = new TH1F("Npass", "Number of Events Process: No Real Photons or BJets", 1, -0.5, 0.5); 
  ...
  // EFficiencies for jets faking bjets
...
//            << " | " << p->motherPdgID << "\n";
    ...
    // Discount all events with any more than one photons, two BJets, or charm jets.
    ...
    //***********************************************************
    // Starting with the Photon
    //***********************************************************  

    //select genjet for a photon
	...
	if (genjet3 == photon2) continue;  // exclude the genjets that are photons
	if (cmsana::deltaR(photon1->eta, photon1->phi, genjet3->eta, genjet3->phi) < 0.5) continue;
	...
	for(Int_t m=k+1; m<genjetArr->GetEntriesFast(); m++) {
	  ...
	  if (genjet4 == photon2) continue;
	  if (cmsana::deltaR(photon1->eta, photon1->phi, genjet4->eta, genjet4->phi) < 0.5) continue;
	  // BJet cuts
	  ...
	  for(Int_t a=0; a<genjetArr->GetEntriesFast(); a++) {
	    ...
	    if ( othergenjet == photon2 || othergenjet == bjet1 ||othergenjet == bjet2) continue;
