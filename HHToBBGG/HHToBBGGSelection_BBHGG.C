...
//
//root -l -b -q  CMSAna/HHToBBGG/HHToBBGGSelection_BBHGG.C+'("root://eoscms//eos/cms/store/group/phys_higgs/future/sixie/Madgraph/bbH_M60To200_14TeV/BACON/BACONNtuple_GenOnly_bbH_M60To200_14TeV_1.root","HHToBBGG.BBHGG.root",20)'
...
double GetSmearedPhotonPt( double pt, double eta, vector<double> PtBins, vector<double> EtaBins, vector<vector<TH1F* > > photonResponseFunctions) {
  
  Int_t PtBin = -1;
  Int_t EtaBin = -1;
  
  if (!(pt > 25 && fabs(eta) < 2.5)) return 0;

  if (pt > PtBins[0] && pt <= PtBins[1]) PtBin = 0;
  if (pt > PtBins[1] && pt <= PtBins[2]) PtBin = 1;
  if (pt > PtBins[2] && pt <= PtBins[3]) PtBin = 2;
  if (pt > PtBins[3] && pt <= PtBins[4]) PtBin = 3;
  if (pt > PtBins[4] && pt <= PtBins[5]) PtBin = 4;
  if (pt > PtBins[5] ) PtBin = 5;
  
  if (fabs(eta) > EtaBins[0] && fabs(eta) <= EtaBins[1]) EtaBin = 0;
  if (fabs(eta) > EtaBins[1] && fabs(eta) <= EtaBins[2]) EtaBin = 1;
  if (fabs(eta) > EtaBins[2] && fabs(eta) <= EtaBins[3]) EtaBin = 2;
  if (fabs(eta) > EtaBins[3] && fabs(eta) <= EtaBins[4]) EtaBin = 3;
  if (fabs(eta) > EtaBins[4] && fabs(eta) <= EtaBins[5]) EtaBin = 4;
  if (fabs(eta) > EtaBins[5] && fabs(eta) <= EtaBins[6]) EtaBin = 5;
  if (fabs(eta) > EtaBins[6] && fabs(eta) <= EtaBins[7]) EtaBin = 6;
  if (fabs(eta) > EtaBins[7] && fabs(eta) <= EtaBins[8]) EtaBin = 7;
  if (fabs(eta) > EtaBins[8] && fabs(eta) <= EtaBins[9]) EtaBin = 8;
  if (fabs(eta) > EtaBins[9]) EtaBin = 9;
  
  assert(PtBin >= 0 && PtBin < PtBins.size());
  assert(EtaBin >= 0 && EtaBin < EtaBins.size());

  double smearedPt = photonResponseFunctions[PtBin][EtaBin]->GetRandom()*pt + pt;
  //cout << "genPt = " << pt << " | smeared Pt: " << smearedPt << "\n";
